...
template<class T>
MatrixClass<T> MatrixClass<T>::operator*(double const &value) const {
	return array.times(value);
}
...
template<class T>
MatrixClass<T> MatrixClass<T>::operator-(MatrixClass const &m) const {
	return array.minus(m);
}

template<class T>
MatrixClass<T> MatrixClass<T>::hadamard(MatrixClass<T> const &m) const {
	assert(height == m.height && width == m.width);
	MatrixClass result(height, width);
